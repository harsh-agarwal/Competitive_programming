...
int main()
{
    ...
    //xor1 = 130 this step is correct

    long long int last_bit_set = xor1 & ~(xor1-1);


    long long int xor2 = 0;
    for(long long int i=1; i<=n;i++)
    {
        long long int last_bit_set_in_numbers = i & ~(i-1);
        if( last_bit_set_in_numbers & last_bit_set)
        {
            xor2 = xor2 ^ i;
            if(i==213) cout<<"hai";
        }
    ...
    for(long long int i=0;i<n;i++)
    {
        long long int last_bit_set_in_numbers = A[i] & ~(A[i]-1);
        if( last_bit_set_in_numbers & last_bit_set)
            xor2 = xor2 ^ A[i];
    }
    ...
    for(long long int i=0;i<n;i++)
    {
        long int last_bit_set_in_numbers = A[i] & ~(A[i]-1);
        if( !(last_bit_set_in_numbers & last_bit_set))
            xor2 = xor2 ^ A[i];
    }
